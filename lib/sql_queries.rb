...
"SELECT projects.title, SUM(pledges.amount) AS total_pledges FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title;"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
"SELECT users.name, users.age, SUM(pledges.amount) AS total_pledges FROM users JOIN pledges ON users.id = pledges.user_id GROUP BY users.name;"
end
...
"SELECT users.name, SUM(pledges.amount) AS total_pledges FROM users JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY SUM(pledges.amount);"
end
